...
#------------------------------------callbacks--------------------------------------------#
	...
	if logger:
		csv_logger = CSVLogger('training_from_scratch_'+'Inc_v3_'+str(nb_epochs)+'.log', separator=',', append=False)
	if lr_reduce:
...
#-----------------------------Actual Training-------------------------------------------#
	...
	metrics_file = open('Inc_v3_CNN_CKplus_reload_v_data_augmented_metrics.txt', 'a')
	metrics_file.write("\nloss: "+str(float(loss))+"\n")
